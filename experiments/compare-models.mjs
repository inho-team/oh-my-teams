/** Historical three-model/two-problem baseline runner. */
...
/** Historical two-problem model baseline retained for reproducibility. */
export const problems = [
  {
    id: "evidence-cache",
    prompt:
      `Implement an ES module exporting function reusable(saved, current). Return true only if both ` +
      `are non-null objects, saved.status is exactly "passed", both head and base are non-empty ` +
      `strings and equal on both objects, environment is a non-empty string and equal, commands are ` +
      `non-empty arrays of non-empty arrays of strings, and commands match element by element in ` +
      `order. Do not mutate inputs. Invalid input returns false, never throws. No imports or side ` +
      `effects.`,
...
console.log('all evidence-cache acceptance cases passed');`,
  },
  {
    ...
    prompt:
      `Implement an ES module exporting function validateOrg(roles). Valid input is a non-null ` +
      `non-array object with exactly five own keys pm, pl, senior, junior, intern. Each value is a ` +
      `non-null non-array object containing a parent property and a positive integer concurrency no ` +
      `greater than 32. pm.parent must be null. Every other parent must be one of those five role IDs, ` +
      `may not equal itself, and following parents must terminate at pm with no cycle. Branching is ` +
      `allowed, so senior and junior may both report to pl. Return {valid:true,errors:[]} when valid, ` +
      `otherwise {valid:false,errors:[at least one non-empty string]}. Never throw for invalid inputs; ` +
      `do not mutate input. No imports or side effects.`,
...
async function main() {
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    assert(
      result.worktree?.path && result.worktree?.id,
      "Missing worktree receipt",
    );
  ...
  await Promise.all(
    records.map(async (record) => {
      for (const problem of problems) {
        ...
        const prompt =
          `Return only a JSON object {"code":"complete ES module source"}. Do not use tools, inspect ` +
          `files, run commands, or edit files. Work entirely from this specification.\n\n${problem.prompt}`;
        const profile = {
          provider: "agy",
          command: ["agy"],
          model: record.model,
          account: "current",
          subscription: "current Agy subscription",
        };
        ...
        let test = { code: -1, stdout: "", stderr: "No valid code" },
          parseError = null;
        try {
          assert(
            result.code === 0 && !result.providerError && !result.timedOut,
        ...
        writeJSON(path.join(dir, "measurement.json"), summary);
        ...
        for (const file of ["solution.mjs", "acceptance.mjs"]) {
