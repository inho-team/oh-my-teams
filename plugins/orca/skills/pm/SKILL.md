...
2. **PL 세우기** — 조각마다 `~/.claude/skills/orca-lead/lead-brief-template.md` 로 PL 브리프 → `launch-worker.sh pl-<조각> <브리프> codex --repo <프로젝트> --handles ~/.<프로젝트>/coord/leads.txt`. 시작할 때 사용자에게 **`/rename pm-<프로젝트>`** 를 한 번 쳐 달라고 한다(Claude Code 가 자동 요약으로 탭 제목을 덮어쓰므로 `orca terminal rename` 은 안 남는다 — 실측 2026-09-13. `/rename` 은 세션 이름을 탭 제목으로 고정한다. 새로 열 때는 `claude -n pm-<프로젝트>`). 기계 부하(`uptime`·`memory_pressure`)를 보고 동시 PL 수를 정한다.
3. **감시(PM)** — **Monitor 도구**(persistent) 로: `while true; do python3 -u ~/.claude/skills/orca-lead/poll.py --run <PM Run> --handles <leads.txt> --seen <seen> --no-pr --max-min 60 2>&1 | grep -E --line-buffered "깨운다|손봐야|·|🔴|오류|조용"; sleep 5; done` — 깨울 줄마다 알림이 오고 스스로 재가동한다(PR 변화가 아니라 PL 의 보고로만 깬다). ⚠️ Bash `run_in_background` 로 폴러를 돌리면 1분 뒤 셸째 조용히 죽는 것을 실측했다(5회, nohup 도 소용없음) — Monitor 를 쓴다. PL 의 `question` 에 답하고, "머지 준비됨" 이 오면 4.
