...
6. **기록** — 백로그·HANDOFF·메모리. 새 함정은 세 스킬 중 맞는 곳에 즉시 적는다. 조각마다 `python3 ~/.orca-skills/orca-lead/agent-stats.py <PL 워크트리 이름>` 의 「백로그 한 줄」 을 표 비고에 적는다(PL 턴이 PR 당 2~3 을 넘으면 잡무가 다시 올라온 것). 인턴을 쓴 조각은 백로그 표 비고에 `인턴 1회 통과` / `인턴 반려→승격` 을 적어 통과율을 모은다(처음 3~5건 뒤에 층 유지 여부를 사용자와 정한다).
