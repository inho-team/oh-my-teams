...
스크립트는 `~/.claude/skills/orca-lead/`(launch-worker.sh·poll.py·verify-pr.sh)를 그대로 쓴다.

## 0. 호칭 (사용자 결정 2026-09-12)
- **PM** = 나(Claude, `orca-top`). **PL** = 조각당 codex 리드(`orca-lead`). **인턴#N(기능)** = 기계적 편집당 agy gpt-oss-120b `-p`(`intern-run.sh` 가 검증·재시도 3회·커밋·PR·보고, 실패면 승격). **주니어#N(기능)** = 일감당 agy-flash(구현). **시니어#N(검토)** = PR 당 agy-pro(검토; 계획형 일감도 시니어) — 브리프 제목·핸들 파일·보고 제목에 이 호칭을 쓴다. 예: `PL — 자원 관제 ②③`, `주니어#1(조치 API)`, `시니어#1(검토)`.
...
2. **리드 세우기** — `launch-worker.sh <조각이름> <리드브리프> codex --repo <프로젝트> --handles <리드핸들파일>`(codex 는 `--supervise` 기본). 리드 브리프는 `lead-brief-template.md`(판단만 — 시니어를 `senior --supervise` 로 세우라는 지시가 들어 있다). PM 은 리드 브리프에 조각 목표·근거·워커 후보·GUARD_CMD 를 적고, 시니어 브리프는 PL 이 `ops-brief-template.md` 로 옮겨 쓴다.
...
6. **기록** — 백로그·HANDOFF·메모리. 새 함정은 세 스킬 중 맞는 곳에 즉시 적는다. 조각마다 `python3 ~/.claude/skills/orca-lead/agent-stats.py <PL 워크트리 이름>` 의 「백로그 한 줄」 을 표 비고에 적는다(PL 턴이 PR 당 2~3 을 넘으면 잡무가 다시 올라온 것). 인턴을 쓴 조각은 백로그 표 비고에 `인턴 1회 통과` / `인턴 반려→승격` 을 적어 통과율을 모은다(처음 3~5건 뒤에 층 유지 여부를 사용자와 정한다).
