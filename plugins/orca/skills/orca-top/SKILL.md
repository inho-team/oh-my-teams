...
- **PM** = 나(Claude, `orca-top`). **PL** = 조각당 codex 리드(`orca-lead`). **인턴#N(기능)** = 기계적 편집당 agy gpt-oss-120b `-p`(`worker-run.sh` 가 검증·재시도 3회·커밋·PR·보고, 실패면 승격). **주니어#N(기능)** = 일감당 agy-flash(구현). **시니어#N(검토)** = PR 당 agy-pro(검토; 계획형 일감도 시니어) — 브리프 제목·핸들 파일·보고 제목에 이 호칭을 쓴다. 예: `PL — 자원 관제 ②③`, `주니어#1(조치 API)`, `시니어#1(검토)`.
