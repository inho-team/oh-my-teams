...
#              끝까지 못 본다(실측 2026-09-13: PL 이 오류로 착각하고 10턴을 썼다). codex/PL 은 `… --notify <내 터미널> > /tmp/launch-<이름>.log 2>&1 &` 로 띄우고 프롬프트로 돌아간다. [--account <계정>]
#   --supervise = 이 에이전트가 **아래 워커를 세우고 기다리는 자리**다(2026-09-13 배치: PL=codex 는 판단만, 시니어=agy-pro 가 운영+검토).
...
#   --run <RUN_ID> = 워커가 보고할 Run(PL 이 run-create 로 만든 것). 워커(agy·claude)에는 **필수** — 공통 규칙의 보고 주소가 된다.
...
#   codex(PL) 분기의 추가 동작(2026-09-12 폴러 역전): PL 터미널에서 워커용 Run 을 먼저 만들고(run-create → 그 터미널에 묶인다),
#     TASK.md 끝에 「자동 배정」(Run id·워커 핸들 파일·본메일 파일)을 붙인 뒤 codex 를 띄우고, PL 워크트리에 **POLLER 터미널**을 하나 더 만들어
#     `poll.py --wake <PL 터미널> --loop` 를 돌린다. PL 은 폴러를 부르지 않는다 — 사건이 오면 폴러가 PL 터미널에 한 줄을 보내 깨운다.
...
if [ "$AGENT" = codex ] && [ -z "$SUPERVISE" ]; then SUPERVISE=1; fi   # codex(PL)는 기본으로 감독 자리
...
#   - 시그널은 자기 프로세스 그룹에만 → 운영 오케·다른 워커·PL 을 kill/pkill/killall 할 수 없다(EPERM). 실측: 하루에 4번 운영을 죽였다.
...
#   좋은 예: pl-jwt-clock, senior-jwt-clock, junior-jwt-clock, junior-jwt-clock-2(재시도), intern-jwt-clock-docs
#   나쁜 예: junior-1, intern-1 (조각이 안 보이고, 이전 조각과 충돌해 -2 경로가 생겼다 — 실측)
case "$AGENT" in codex) WANT=pl;; senior|agy-pro|agy) WANT=senior;; junior|agy-flash) WANT=junior;; intern) WANT=intern;; claude) WANT="";; esac
...
case "$AGENT" in
  agy|agy-pro|senior)   CMD='agy --model gemini-3.1-pro-high --dangerously-skip-permissions';   ALIVE='Gemini\|shortcuts';;
  agy-flash|junior)     CMD='agy --model gemini-3.8-flash-high --dangerously-skip-permissions'; ALIVE='Gemini\|shortcuts';;
  intern)               CMD='agy --model gpt-oss-120b-medium --dangerously-skip-permissions';  ALIVE='GPT-OSS\|gpt-oss\|shortcuts';;   # 상태줄 실측 2026-09-13: '? for shortcuts … GPT-OSS 120B (Medium)'
  claude)        CMD="claude -n '$NAME' --dangerously-skip-permissions";                  ALIVE='Model:\|bypass permissions';;   # -n: 세션 이름=탭 제목(자동 요약 제목이 덮어쓰지 않는다)
  codex)         CMD='codex --model gpt-5.6-sol --dangerously-bypass-approvals-and-sandbox'; ALIVE='Ask Codex to do anything\|gpt-5';;   # PL(리드)용. 이 계정은 gpt-5.4 가 400 이라 5.6-sol
  *) echo "🔴 모르는 에이전트: $AGENT (intern | junior | senior | agy-flash | agy-pro | codex | claude)"; exit 1;;
esac
...
# 전용 DB 등을 올리고 KEY=VALUE 출력을 받는다. 워커·PL 은 docker 를 만지지 않고 받은 주소만 쓴다(seatbelt 가 docker 소켓을 막는다).
...
case "$AGENT" in agy|agy-pro|agy-flash|intern|junior|senior|claude) IS_WORKER=1;; *) IS_WORKER=0;; esac
if [ "$IS_WORKER" = 1 ]; then
...
#   --account <이름> 이 없으면 project.env 의 ACCOUNT_<역할>(JUNIOR/SENIOR/INTERN/PL/CLAUDE) 을, 그것도 없으면 기본 계정을 쓴다.
...
# --- 감독 자리(--supervise: codex PL 기본, 시니어 운영): 워커용 Run 을 이 터미널(셸 상태)에서 만들어 묶고, 「자동 배정」 을 TASK.md 에 붙인다 ---
...
if [ "$SUPERVISE" = 1 ] && [ -n "$POLL_RUN" ]; then
  ...
  if [ -n "$PT" ]; then
    ...
    echo "⚠️ POLLER 터미널을 못 만들었다 — PL 에게 손으로 폴러를 돌리라고 해야 한다"
  fi
fi
