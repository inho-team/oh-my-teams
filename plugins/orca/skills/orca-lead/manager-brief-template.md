...
2. **PR 도착** — 먼저 사원에게 초안을 시킨다(각 20~30초, 인용은 대조기가 실제 파일과 맞춘 것만 온다. 저장소는 **절대 경로**):
   ...
   🔴 **`verify-pr.sh` 는 반드시 네가 돌린다.** 대리에게 "가드 빼고 돌린 출력·변이 출력을 보내라" 고 시키지 마라 — 구현자가 자기 검증을 하는 것이라 독립성이 없다(실측 2026-09-13 weblog-timesource: 과장이 verify 를 한 번도 안 돌리고 대리의 출력을 묶음에 넣었다. 이사 표본에서만 독립 검증됨). 사원 초안이 비어 나와도(도구 오류) 변이는 브리프의 힌트로 네가 쓰고 verify 를 돌려라.
