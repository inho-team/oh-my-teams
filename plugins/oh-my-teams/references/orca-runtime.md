...
`/clear`를 보내기 전에는 `clearRoleTerminal`이 `worker-list`로 그 터미널에 아직 남아 있는 Dispatch가 있는지 먼저 확인한다(#84). `dispatchStatus`가 `dispatched`인 Dispatch가 나오면 그 dispatch id와 task id를 알리며 `/clear`를 보내지 않고 거부하고, `worker-abandon`으로 정리한 뒤 다시 시작하도록 안내한다. `worker-list` 호출이 실패하거나 응답으로 판정할 수 없으면 활성 Dispatch가 없다고 단정하지 않고, `/clear`만 건너뛴 채 `{cleared: false, reason: "active-dispatch-unknown"}`을 돌려준다. 이 경우 전체 시작을 거부하지는 않으며, 터미널은 비워지지 않은 채로 다음 작업을 받는다. 활성 Dispatch가 전혀 없을 때에만 기존과 같이 idle 확인, `/clear`, idle 재확인 순서를 그대로 밟는다.

`worker-start --terminal`이 돌려준 영수증이 `ready: true`이면서 `result.prompt.stages`에 `turn_started`가 없으면(#87), `confirmWorkerSubmission`(prompt-submission.mjs)이 그 터미널의 화면을 한 번 읽어 판정한다. `worker-start`가 입력 상자에 타이핑하는 텍스트는 승인한 spec·task 원문이 아니라 Orca 자신의 preamble로 감싼 텍스트이므로, 위 「프롬프트 전달과 제출 확인」 절의 정확한 텍스트 비교는 그대로 쓸 수 없다. 대신 Dispatch의 task id가 화면 기록에 보이는지로 판정한다. 입력 상자에 텍스트가 남아 있으면 `unsubmitted`로 보고 Enter를 한 번만 보내고, 입력 상자가 비었고 task id가 기록에 보이면 `already-started`로 아무것도 보내지 않으며, 어느 쪽도 아니면 `unclear`로 남기고 다시 보내지 않는다. 판정 결과는 영수증의 `submission` 필드(`outcome`, `reason`, `enterSent`)에 남는다.
...
같은 워크트리에 대해 `director-signal`이 겹쳐 실행되는 경우(예: 앞선 호출이 `deliverPrompt`의 `--wait-submit` 대기 중일 때 다음 호출이 시작하는 경우)에는, 이번에 보낼 묶음(미배달 백로그와 이번 신호)을 실제로 터미널에 보내기 전에 pid·hostname으로 선점 표시부터 남긴다. 다른 호출이 이미 선점한 신호가 하나라도 있고 그 소유자가 죽었다고(`processLiveness`로) 증명되지 않으면 터미널을 전혀 건드리지 않고 `notified: false, deferred: true, notifyError: "backlog-claimed"`로 물러난다. 선점 표시는 전송을 마친 뒤 실제 결과로 덮어써 지워지므로, 비정상 종료로 선점 표시만 남았을 때는 다음 호출이 `processLiveness`로 그 pid가 죽었음을 확인하고 이어받는다.
