...
1. `role-command`와 같은 명령으로 터미널을 만든다. `scripts/launch-matrix.mjs`의 호환성 표가 `supervised-terminal`을 돌려주는 Agy 역할 중 POSIX 셸에서는 `stty cols 44`를 앞에 붙인다. 이 폭 조정은 Orca 1.4.210에서 폭 조정 없이도 감독 터미널이 성립하는 것을 실측으로 확인했으나([#104](https://github.com/inho-team/oh-my-teams/issues/104)), 이 설정을 유지하고 있다. Windows에서는 폭 조정을 생략한다(`role-terminal.mjs`의 폭 조정 조건; `docs/plan/agy-terminal-path.md`의 「설계」 7절).
