...
제한 편집 하네스는 `scripts/providers.mjs`가 프로필의 모델 ID를 인자 배열에 추가한다. 일반 감독 작업에서 Orca가 Agy를 아는 agent ID는 `agy`가 아니라 `antigravity`이며, `worker-start --model`은 Claude·Codex·Cursor에만 적용된다. 따라서 `worker-start` 래퍼는 Agy 프로필을 거부하고 이 절을 가리킨다. 이때에는 현재 orchestration 가이드의 custom argv 경로를 사용하되, 모델을 감지할 수 없거나 대화형 화면의 현재 모델이 요청과 다르면 Dispatch를 시작하지 않는다. 계정·모델을 다른 실행기로 임의 대체하지 않는다.
...
감독 worker는 역할 이름과 조직 파일로만 시작한다. 원시 `orca orchestration worker-start`로 `--agent`와 `--model`을 손으로 적지 않는다. 손으로 적은 명령은 저장된 모델을 빠뜨려도 아무 오류 없이 계정 기본 모델로 실행되고, 보고서는 여전히 역할의 프로필을 적기 때문이다.

```text
npm run org -- worker-start --org <organization.json> --role <pl|senior|junior|intern> --repo <coordinator-worktree> --spec <작업> [--worktree new-child] [--run <runId>]
```

래퍼는 역할을 서열에 따라 접어 확정한다. `--workflow-id`와 `--state`를 주면 그 workflow가 기록한 이번 실행의 역할 목록으로 접고, 주지 않거나 목록이 없으면 조직이 선언한 역할로 접는다. 그다음 그 프로필에서 Orca agent(`claude` 또는 `codex`), `--model`, `--effort`를 정한다. `--agent`, `--model`, `--effort`를 함께 적으면 프로필과 같을 때에만 받아들이고, 다르면 Orca를 호출하기 전에 거부한다. 다음 경우에도 호출 전에 거부한다.

- 역할이 PM이거나, 선언되지 않은 역할이 PM으로 접힌다. PM은 coordinator이며 아래 「coordinator 실행」으로 띄운다.
- 프로필이 Agy·Ollama다. Orca가 모델을 고정해 띄울 수 없으므로 「Agy 모델 선택」의 custom argv 경로를 따른다.
- 프로필이 현재 계정이 아니거나, `env` 또는 추가 인자로 계정을 고른다. Orca agent ID는 실행 파일만 가리키므로 계정을 표현하지 못한다.
- 프로필이 모델 없이 강도만 기록한다. Orca는 `--model` 없는 `--effort`를 거부한다.

`--spec` 앞에는 받는 역할의 머리글이 붙는다. 머리글에는 역할, 보고 대상, 이번 실행에 없어 이어받는 역할, 직접 배정할 수 있는 역할, 그리고 역할 스킬의 `권한·책임·한계` 절 전문이 들어간다. `task-create`로 만든 Task를 `--task`로 시작할 때에는 래퍼가 머리글을 붙일 수 없으므로, Task 설명을 `role-spec --org <organization.json> --role <역할> --spec <작업>`의 출력으로 만든다.

시작 결과의 `binding`에는 `modelRequested`, `effortRequested`, `modelProof`가 남는다. `modelProof`는 다음 넷 중 하나다.

| 값 | 뜻 | 다음 행동 |
|---|---|---|
| `matched` | receipt의 `launch.effective.model`이 요청과 같다. | 화면의 모델을 대조한 뒤 계속한다. |
| `mismatched` | Orca가 다른 모델로 띄웠다고 기록했다. 출력에 `status: "blocked"`가 붙고 종료 코드는 1이다. | 추가 지시를 보내지 않고 결과를 채택하지 않으며, 아래 실패 복구 절차와 함께 상위에 보고한다. |
| `unproven` | receipt에 `launch.effective`가 없거나 기존 터미널을 재사용했다. | 화면에서 모델을 확인하기 전에는 요청 모델로 실행 중이라고 보고하지 않는다. |
...
`launch.effective`는 Orca가 적용한 실행 인자의 기록이지 모델이 스스로 보고한 값이 아니다. 그러므로 `matched`여도 시작 직후 `worker-read --dispatch <id> --source terminal`이나 `terminal read --screen`으로 대화형 화면에 표시된 현재 모델을 확인하고, `modelRequested`와 다르면 추가 지시를 보내지 않고 상위에 보고한다.
...
`--terminal`로 기존 터미널을 재사용할 때에는 Orca가 `--model`과 `--effort`를 받지 않으므로 래퍼도 전달하지 않는다. 재사용한 터미널은 처음 띄울 때의 모델을 유지하므로 `modelProof`는 `unproven`으로 남는다.
...
`role-command`는 Claude에는 `claude --model <model>`, Codex에는 `codex --model <model> --config model_reasoning_effort=<effort>`를 만들고, 모델이 `null`이면 모델 인자 없이 만든다. 괄호처럼 셸이 해석하는 문자가 든 모델 ID는 작은따옴표로 감싼다. 브리프를 보내기 전에 화면에 표시된 모델이 `modelRequested`와 같은지 확인한다. `role-command`가 프로필을 거부하거나 화면의 모델이 다르면 브리프를 보내지 않고 사용자에게 보고한다. 이 경우 다른 실행기나 기본 모델로 대신 띄우지 않는다.
...
2. 마지막 heartbeat, 메시지, 출력 변화 가운데 가장 최근 시각을 `lastActivityAt`으로, 그 뒤로 답을 받지 못한 진행 요청 수를 `unansweredRequests`로 적어 `node <runtime> supervision-next --org <organization.json> --observation <observation.json>`을 실행한다.
3. 결과의 `action`대로 행동한다.

| action | 행동 |
|---|---|
| `wait` | 다시 `check --wait`로 기다린다. |
...
| `inspect` | `worker-show`와 `worker-read --dispatch <id> --source auto --limit <n>`으로 상태와 최근 출력을 확인한 뒤 관측값을 고쳐 다시 판정한다. |
| `escalate` | `worker-read`의 제한된 출력, liveness, 무응답 시간과 보낸 요청을 증거로 붙여 상위에 보고한다. PL은 `orchestration send --type escalation`으로 PM에게, PM은 사용자에게 보고한다. `failureClassify`가 `true`이면 그 증거로 `failure-classify`를 실행한다. |
