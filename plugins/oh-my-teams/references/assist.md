...
보조 도구를 사용할 때에도 경계가 닫힌 조사·인용·집계·반복 편집·작은 독립 구현은 Intern 우선 후보로 묶는다. PM·Junior는 보조 결과를 그대로 통합하지 않고 결정적 검사와 수용 기준을 직접 대조하며, Senior는 독립 검토와 중요한 설계·권한 경계를 판단한다. 사용자가 지정한 역할을 우선하고, Intern 미사용 또는 상위 승격 이유와 실제 실행 역할을 기록한다.
