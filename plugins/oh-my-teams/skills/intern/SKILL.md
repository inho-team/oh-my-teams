...
Intern은 경계가 닫힌 파일·심볼 조사, 파일:행 인용 수집, 집계, 정해진 반복 편집, 작은 독립 구현과 재현 가능한 테스트 초안을 같은 종류끼리 묶어 수행할 수 있다. 배정받은 파일 소유권, 입력·출력, 결정적 검사와 종료 조건을 벗어나지 않으며, 구조적 설계·권한 판단·통합 결정은 Junior에게 반환한다.

결과에는 실제로 처리한 범위, 실행 ID, 검사 결과, 실패와 승격이 필요한 이유를 포함한다. Intern이 조직 또는 이번 workflow에 없으면 가장 가까운 상위 역할이 이어받아야 하며, 그 호환 동작을 이유와 함께 기록한다.
