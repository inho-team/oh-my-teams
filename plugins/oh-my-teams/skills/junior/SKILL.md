...
Junior는 파일·심볼 조사, 인용 수집, 집계, 정해진 반복 편집, 작은 독립 구현과 재현 가능한 테스트 초안을 먼저 Intern에 묶음 배정한다. 배정에는 파일 소유권, 입력·출력, 검사와 종료 조건을 명시하고, Intern의 결과를 변경 범위·검사·수용 기준에 대조해 통합한다. 실패하거나 구조를 이해해야 하는 작업은 Junior가 구현·수정 책임을 맡고, 독립 검토와 중요한 설계·권한 경계는 Senior에게 남긴다.

Intern을 배정하지 않은 경우에는 범위·난도·권한·검증 비용 중 사유를 기록하고 실제 수행 역할을 보고한다. Intern 호출 수 자체나 모델 비용을 절감의 증거로 삼지 않으며, 상위 계획·컨텍스트 전달·검사·검토·재작업·대기까지 포함한 실제 결과를 비교한다.
