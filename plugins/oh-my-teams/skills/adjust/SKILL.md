...
조직을 조정할 때 Intern의 선언 여부는 이후 경계가 닫힌 저위험 실무를 우선 배정할 수 있는지에 영향을 준다. 역할을 제거하거나 추가해도 기존 workflow와 스냅샷의 의미는 바꾸지 않으며, Intern이 없는 조직에서는 가까운 상위 역할로 이어받은 사실을 실행 기록에 남긴다.
