...
조직을 저장할 때 Intern이 선언되어 있으면 이후 kickoff에서 경계가 닫힌 조사·집계·반복 편집·작은 독립 구현을 Intern에 우선 배정할 수 있도록 역할 설명을 보존한다. 이 기본값은 사용자의 명시적 역할 선택을 덮지 않으며, Intern이 없는 조직과 기존 workflow도 호환되게 유지한다.
