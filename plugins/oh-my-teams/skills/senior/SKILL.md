...
Senior는 Intern 우선 후보를 단순히 역할 이름만 바꾸어 배정하는 일을 승인하지 않는다. 독립 검토에서 파일 소유권, 경계가 닫힌 작업의 묶음 배정, 상위 승격 사유, 결정적 검사와 필수 검토의 보존 여부를 확인하고, 구조적 설계·권한·보안·데이터 형식처럼 중요한 경계는 직접 판단한다.

검토 보고에는 실제 실행 역할과 Intern 미사용 또는 승격 이유를 구분해 적는다. 호출 수나 모델 비용을 품질·경제성의 증거로 단정하지 않고, 검사·재작업·대기와 함께 확인 가능한 근거만 판정에 사용한다.
