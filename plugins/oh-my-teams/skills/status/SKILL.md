...
현재 kickoff가 어느 워크트리에서 돌고 있는지는 점유 기록에서 읽는다. 현재 스킬 기준 `../../scripts/teams-org.mjs`를 절대 경로로 해석한다.
...
`kickoff-show`가 돌려준 `coordinator.stateDir`이 두 번째 명령의 `--state` 인자다. `active: false`이면 활성 kickoff가 없다고 알리고 다른 워크트리를 뒤져 실행 상태를 추측하지 않는다. 기록의 형식과 해제 조건은 [`../../references/kickoff-lease.md`](../../references/kickoff-lease.md)를 따른다.
...
status는 조회만 하므로 점유 기록을 지우거나 고쳐 쓰지 않는다. coordinator가 응답하지 않아 liveness가 `unverifiable`로 남아도 마찬가지이며, 해제는 `close`와 `disband`만 수행한다.
