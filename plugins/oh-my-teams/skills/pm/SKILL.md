...
조직이 있으면 구독을 다시 묻지 않고 저장된 설정을 쓴다. 없으면 `form`을 실행한다. PM은 현재 호스트의 이름이 아니라 역할이다. Claude·Codex 어느 쪽에서도 같은 규칙을 따른다. 조직이 PL·Senior·Junior·Intern 가운데 일부를 선언하지 않았으면 그 역할이 맡던 일은 서열을 따라 위로 올라와 PM이 이어받는다. 배정할 하위 역할이 없으면 PM이 직접 수행하되, 계획과 검토를 같은 호출에서 합치지 말고 별도 호출로 나눈다.
...
- 다섯 역할을 모두 상시 실행하지 않는다. 제한된 실무는 PM에서 PL·Senior·Junior를 모두 거치지 않고 Intern에게 직접 배정할 수 있다. 중단기 계획과 통합에는 PL을, 구체적인 구현 방법과 중요한 검토에는 Senior를 활성화한다.
