...
PM은 파일·심볼 조사, 인용 수집, 집계, 정해진 반복 편집, 작은 독립 구현과 재현 가능한 테스트 초안을 경계가 닫힌 작업으로 분류하고, Intern이 이번 실행에 있으면 같은 종류를 한 task로 묶어 가장 낮은 역할에 먼저 배정한다. 배정에는 파일 소유권, 입력·출력, 결정적 검사와 종료 조건을 적고, PM은 하위 결과의 요약·실패·중요 경계와 검사 결과를 우선 확인하며 모든 원문을 다시 읽지 않는다.

난도·권한·위험이 크거나 독립 검토가 필요한 작업은 Senior 또는 Junior에 남긴다. Intern을 배정하지 않으면 범위·난도·권한·검증 비용 가운데 이유를 task와 진행 기록에 남기고, 실제 실행 역할과 상위 승격 사유도 확인한다. 사용자가 지정한 역할은 이 분류보다 우선한다. 이 규칙은 기존 계약, 예산, 필수 검토와 정식 wrapper 경로를 바꾸지 않으며, 모델별 비용 절감은 측정 전 가설로만 보고한다.
...
- 전체 요청을 목표·수용 기준·비목표·제약과 파일 소유권이 분명한 task v2로 나눈다. 일반 계획은 [`../../examples/task.v2.json`](../../examples/task.v2.json)처럼 `delegation`의 범위·검증·권한·설계를 모두 채우고, [`../../examples/workflow.json`](../../examples/workflow.json)처럼 `tasks`의 `role`은 생략하여 런타임 자동 선택을 사용한다. 사용자가 역할을 명시하면 해당 `role`이 자동 선택보다 우선하며, 선택된 역할과 같은 `--role`로 `role-terminal`·`worker-start` 래퍼를 실행한다. 작은 저위험 변경은 한 task로 유지한다. 독립 편집 작업마다 **Orca child worktree**를 사용한다. 기준 커밋을 명시하고 실제 반환된 전체 worktree ID를 보관한다. 다른 역할의 task가 작업하는 워크트리에 역할을 띄우지 않으며, 검토자는 검토 대상을 경로와 커밋으로 읽게 한다([`../../references/orca-runtime.md`](../../references/orca-runtime.md)의 `역할과 워크트리` 절).
