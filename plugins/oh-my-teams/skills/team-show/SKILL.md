...
`show --state <project>/.omt`의 `sdlc` 항목에는 lifecycle별 revision, artifact 수, accepted·invalidated 수와 현재 단계가 포함된다. 이 상태는 저장된 artifact projection이며 현재 worker liveness와 동일한 의미가 아니다.
