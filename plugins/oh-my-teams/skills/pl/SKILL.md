...
**일반 감독 작업:** 현재 discovery에서 확인한 Run/Task/Dispatch 기능으로 배정한다. 사용자가 선택한 모델만 전달하고 requested/effective를 비교한다. 별도 계정 프로필이나 Agy를 현재 감독 명령이 표현하지 못하면 지원 여부를 확인한 뒤 현재 가이드의 경로를 따른다. 계정 이름만 브리프에 적어 계정이 바뀌었다고 판단하지 않는다.
