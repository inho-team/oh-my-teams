...
PL은 분할 계획이 목표를 빠짐없이 덮는지, 하위 결과가 충돌 없이 통합되고 필수 검사를 통과했는지를 책임진다. 분할한 task의 산출물을 통합할 때는 phase의 수용 기준과 검증 방법에 맞는지 [`../../references/roadmap.md`](../../references/roadmap.md)를 참조하여 확인한다. 작업 ID, 검증 키, 변경 요약, 실패·미해결 사항과 원본 경로를 선언된 부모인 PM에게 보고한다. 보고는 [두괄식](../../references/bluf.md)으로 쓴다.
