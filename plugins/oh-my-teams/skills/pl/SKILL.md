...
PL은 의존성과 파일 소유권이 닫힌 조사·집계·인용·반복 편집·작은 독립 구현을 같은 종류끼리 묶어 Intern 우선 task 후보로 만든다. 각 task에는 목표와 완료 조건, 입력·출력, 결정적 검사, 실패 시 상위 승격 조건을 적고, 구조적 설계·통합·중요 경계 검토는 Junior와 Senior의 책임으로 남긴다.

Intern을 이번 실행에서 사용하지 못하거나 사용하지 않기로 하면 그 이유와 실제로 이어받은 역할을 계획 및 통합 기록에 남긴다. PL은 PM에게 원문 전체가 아니라 결정적 검사 결과, 근거 요약, 실패와 중요 경계를 전달하며, 기존 workflow의 state·hash·예산·필수 검토 의미를 바꾸지 않는다.
