...
- **사원은 2인 1조**(2026-09-14). 작업 사원이 완료 조건을 넘기면 검수 사원(gpt-oss)이 diff 를 보고 실행 가능한 검사·변이를 JSON 으로 내고 하네스가 돌린다. 미달이면 임시 커밋을 풀고 실패 출력과 함께 작업 사원에게 되돌린다(일감당 1회) — gpt-oss 할당량으로 Claude·codex·gemini 를 아끼는 구조다. 사다리는 gpt-oss 5회 → 대리급 승격 2회(같은 하네스 안에서 flash 가 편집만), 그래도 실패면 대리 **세션**이 직접 본다(모델은 둘 다 flash, 자리가 다르다). 실측: 검수 검사 4건 31초, 그러나 「빈 줄 두 개」는 못 잡았다 — 검수는 완료 조건의 보강이지 대체가 아니다.
