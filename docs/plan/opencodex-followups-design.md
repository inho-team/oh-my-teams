...
`roleCommand`(`role-launch.mjs:366-408`)는 runner가 있으면 `actualRunner: "codex"`를 명시한 객체를 만듭니다(`role-launch.mjs:375-386`). `launchableProfile`(`role-launch.mjs:152-176`)은 `ORCA_LAUNCH`에 없는 provider를 예외 없이 거부하고(`:153-156`), runner 예외는 그 뒤의 "현재 계정, env 없음, 단일 명령" 검사에만 적용됩니다(`:161-166`).
