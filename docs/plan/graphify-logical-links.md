...
**A·B·C 지점은 보류, D 지점은 CLI 경로 제약(API 키 요구·--code-only 제한)으로 기각한다. 채택 전 추가 검증이 필요한 항목이 있으므로 지금 당장 스킬과 런타임을 수정하지 않는다.**

graphify는 `.mjs` 파일의 `calls`/`imports` 관계를 모델 없이 AST로 추출하며, 실측에서 46초 만에 129개 코드 파일로부터 1,366 노드·3,718 엣지의 그래프를 생성했다. `graphify affected`는 직접 import 파일을 grep 결과와 일치하게 추출하며, 2단계 `imports_from` 경유 간접 의존자를 추가로 포함한다. 마크다운 링크 추출(`extract_markdown`)은 정규식 기반으로 모델이 필요하지 않지만, CLI의 `graphify extract`는 문서 파일에 API 키를 요구하고 `--code-only`는 `.md`를 건너뛰어 CLI만으로는 관계를 `graph.json`에 넣을 수 없다(D 기각). `graphify prs --conflicts`는 graphify 내부에서 gh 인증 오류가 발생해 실행하지 못했다(원인 미확인). OMT가 Node.js 전용 환경이라는 점과 Python 의존성 추가 비용을 고려하면 현 시점에서 필수 의존성으로 채택하기에는 운영 위험이 더 큰 검증이 필요하다.
...
마크다운·PDF·이미지 파일을 포함해 추출하면 Pass 3(LLM 의미 분석)이 실행된다. OMT의 `docs/`와 `plugins/oh-my-teams/`에는 48개 이상의 문서 파일이 있으며, 각 파일이 여러 번의 LLM 호출을 유발할 수 있다. 이 비용은 코드 그래프 생성(0 LLM 크레딧)과 달리 구독 할당량을 소비한다. CLI 경로로 D 지점을 운용하려면 이 비용이 발생하며, 직접 API 호출 경로는 노드 ID 정규화와 코드 그래프 통합 작업이 추가로 필요하다.
