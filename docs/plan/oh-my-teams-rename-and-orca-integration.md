...
- 기존 `.orca` 기록은 자동 이동하거나 삭제하지 않는다. 이후 제품 상태 디렉터리를 분리하려면 별도 migration으로 처리한다.
...
- [ ] 활성 제품 표시명이 `oh my teams`로 일관된다.
- [ ] 두 marketplace·manifest·설치기의 ID와 경로가 일치한다.
- [ ] 기존 조직 설정과 실행 기록이 보존된다.
- [ ] 역할명과 모델·계정 선택이 이름 변경 때문에 바뀌지 않는다.
- [ ] 실제 Orca 명령과 ORCA_* 환경변수는 제품명 치환 대상에서 제외된다.
- [ ] 레거시 기록과 과거 실험 결과를 잘못 고쳐 쓰지 않는다.
- [ ] 로컬 import, CLI help, schema 예제, 기존 런타임 테스트가 통과한다.
- [ ] 최신 가이드는 선택된 바이너리에서 조회하며 repo에 전문을 복제하지 않는다.
- [ ] 가이드 조회 실패·구버전·다른 실행 파일·변경된 응답을 구분한다.
- [ ] 가이드 조회만으로 “최신 바이너리 설치 완료”라고 표시하지 않는다.
- [ ] 실행 중 업데이트와 중복 리소스 생성 시나리오를 검증한다.
- [ ] 설치 이전은 새 plugin 발견을 확인한 뒤 기존 plugin 처리로 진행한다.
- [ ] 제품 문서에 지원되는 경로와 아직 계획인 CLI를 구분한다.
