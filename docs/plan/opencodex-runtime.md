...
완료: ChatGPT·Claude와 추가 Antigravity 두 계정의 동일 픽스처 성공, 선언된 풀의 전환 한 건, 취소·폴더 신뢰 관측을 기록했습니다. 공유 계정의 고정 비교는 사용자 결정에 따라 미실행으로 마감했으며, 전체 공통 실행 경로의 전환 판정은 남은 관측 한계 때문에 보류합니다. 최종 비교에서 기존 Claude 키체인 항목의 메타데이터 변경도 발견했으며 변경 주체는 확인하지 못했습니다.
...
| Agy 공유 계정 `e4a44806…` | 풀 전환 실험에서 같은 Claude 모델을 요청했습니다. | 사용자 예외 승인에 따라 04:09Z에 요청 한 건을 보냈고 OAuth 429 회복 뒤 추가 계정으로 전환되어 성공했습니다. 고정 픽스처 비교는 미실행입니다. 사용자 결정으로 Claude/Agy 추론을 중단했습니다(2026-09-21 05:10Z). |
...
| `e4a44806…` | `/tmp/omt-opencodex-probe.dAKX3U/agy-e4a44806/ocx` | 18476 | 홈만 준비했습니다. 고정 비교는 사용자 결정으로 미실행 마감했습니다(2026-09-21 05:10Z). |
...
세 구독 계열에서 같은 작은 저장소 작업을 수행할 수 있다는 긍정적인 증거가 생겼습니다. 그러나 공통 실행 경로의 전면 전환은 아직 보류합니다. 동시 요청에서 풀 전환을 완전하게 기록할 수 있는지, 공유 계정 고정 비교의 미실행, 신뢰 이후 입력 제출과 상위 요청 진행 중 취소, Windows 실측 및 Senior 독립 검토의 부재가 있기 때문입니다.
...
최초 조사 커밋은 `npm run format`, `npm run sync`, `npm run lint`, `npm test`를 통과했고 전체 테스트는 377개였습니다. Python AST 검사와 별도 임시 복사본의 결함/수동 수정 대조도 통과했습니다. 후속 실측 기록에서도 같은 검사와 전체 테스트 377개를 통과했습니다. 추가 계정의 프록시 두 개는 증거 수집 후 SIGTERM으로 정상 종료했고 각각 종료 코드 0을 확인했습니다. 두 추가 계정 프록시를 종료한 직후인 04:12Z의 비교에서는 조회한 전역 파일과 키체인 메타데이터가 동일했습니다. 원래 18473 프록시도 최종 지시에 따라 SIGTERM으로 종료했고 종료 코드 0을 확인했습니다. 18473~18476 포트에 리스너가 없으며, 알려진 실험 PID와 실험 경로를 인수로 가진 잔존 프로세스도 발견하지 못했습니다. `/tmp/omt-opencodex-probe.dAKX3U`는 삭제하지 않았습니다. 이 검사는 구독 청구서 확인이나 독립적인 Senior 의미 검토를 대신하지 않습니다.

## 최종 중단 결정과 전달 상태

사용자는 2026-09-21 05:10Z에 Claude와 Agy 추론을 더 만들지 않기로 결정했습니다. PM이 05:22:58Z에 이를 전달했으며, 06:04:01Z 이후 예정했던 e4a4 단일 계정 비교는 실행하지 않았습니다. 이미 완료한 네 픽스처 작업과 공유 계정의 예외 승인된 풀 요청 한 건은 그대로 보존했습니다.

최종 정리 후 조회한 전역 설정 파일은 기준값과 같았고 OpenCodex 키체인 서비스 두 개도 계속 없었습니다. 그러나 `Claude Code-credentials`의 메타데이터 SHA-256은 `f6b03eee9499f98153667b83528e04df3764ace87b86d9c931988d0beafb2bd9`로 바뀌었습니다. 해당 항목의 mdat는 2026-09-21T04:57:35Z이며, 마지막 무변경 관측인 04:26:26Z 이후이자 프록시 종료인 05:23:27Z 이전입니다. 이때 작업자는 대기 중이었고 다른 Claude 세션도 있는 환경이므로 변경 주체를 특정할 수 없습니다. 비밀값은 조회하지 않았고 전역 항목을 되돌리지 않았습니다. 따라서 실측 직후의 제한된 무변경 증거는 유지하되 전체 실행 기간의 키체인 무변경까지 입증했다고 주장하지 않습니다. 포트·프로세스 종료와 마지막 스냅샷은 evidence.json의 finalCleanup과 finalSnapshot에 있습니다. 중단은 실험 실패나 해당 구독의 비지원 판정으로 바꾸어 쓰지 않습니다. W2 분할안은 [별도 계획 문서](opencodex-runtime-w2-plan.md)에 있으며, 이 조사에서 구현이나 새로운 추론 작업을 시작하지 않았습니다.
