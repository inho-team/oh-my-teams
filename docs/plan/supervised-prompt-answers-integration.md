...
**첫째 단계: 워크트리 생성과 role-terminal, 그 결과 한도가 막지 않았고 폴더 신뢰 질문은 나타나지 않았다**
...
**둘째 단계: 감독자가 화면을 읽고 답하는 경로, 그 결과 동작했으나 분류기는 답하지 않았다**
...
**셋째 단계: worker 시작, 그 결과 시작 경로는 동작했고 첫 모델 호출이 한도로 막혔다**
