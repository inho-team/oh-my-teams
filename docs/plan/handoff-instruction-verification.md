...
3. `node <runtime> role-terminal --org <위 1단계의 임시 org 파일> --role pm --worktree id:955a83a3-27ee-457a-8ff5-3dff4694e8aa::/Users/jinsungkim/orca/workspaces/oh-my-teams/terminal-delivery-judgment --brief <위 2단계의 확인용 브리프 경로> --title "86 확인"`을 한 번 실행했다.

### 실행 조건

이 rework의 spec은 이 확인을 t6 작업 워크트리(`id:955a83a3-27ee-457a-8ff5-3dff4694e8aa::/Users/jinsungkim/orca/workspaces/oh-my-teams/tdj-t6-handoff`)에서 실행하라고 지시했다. 그러나 위 3단계가 `--worktree`로 받은 값은 같은 uuid의 다른 경로, PM 워크트리(`id:955a83a3-27ee-457a-8ff5-3dff4694e8aa::/Users/jinsungkim/orca/workspaces/oh-my-teams/terminal-delivery-judgment`)였다. `role-launch.mjs`의 `selectedWorktreePath`·`worktreePath`는 `id:` 선택자의 uuid가 아니라 마지막 `::` 뒤 경로 문자열만 resolve해 실제 대상 디렉터리로 쓰므로, 새로 연 PM 세션은 지시받은 t6 워크트리가 아니라 PM 워크트리에서 열렸다. 그 워크트리는 확인 시점에 커밋 `a05fd92`(t6 구현 커밋 `0f8bb8c` 이전)에 있었고, `git show a05fd92:plugins/oh-my-teams/skills/pm/SKILL.md`에는 「인계 지시 확인」 절 자체가 없다(`0f8bb8c`가 처음 추가한 절이므로).

그러나 `--worktree`가 가리킨 대상 디렉터리(새 PM 세션이 열리는 곳)와, `role-terminal` 명령 자체를 실행한 런타임 스크립트는 서로 다른 것이었다. `--brief`는 `a05fd92`의 `teams-org.mjs`에는 없는 옵션이다: `git show a05fd92:plugins/oh-my-teams/scripts/teams-org.mjs`의 `role-terminal` 옵션 목록(427~438행)은 `org`, `role`, `worktree`, `title`, `workflow-id`, `workflow-task`, `profile`, `state`, `orca`, `allow-unverified` 열 개뿐이고 `brief`가 없으며, `git show a05fd92:plugins/oh-my-teams/scripts/role-terminal.mjs`에도 `brief` 문자열이 전혀 없다. 반면 `git show efad711:plugins/oh-my-teams/scripts/teams-org.mjs`의 같은 목록(430~442행)에는 열한 번째 항목으로 `brief`가 있다(`0f8bb8c`가 추가). 실제 확인에서 `--brief`가 그 자리에서 거부되지 않고 명령 결과의 `command`·`launched`에 브리프 문장이 실행 명령의 마지막 인자로 실렸으므로, 이 확인을 실행한 `node <runtime> role-terminal`의 `<runtime>`은 `--brief`를 아는 버전(`0f8bb8c` 이후의 `teams-org.mjs`)이었다. 즉 새로 연 PM 세션의 작업 디렉터리(및 그 안의 `pm/SKILL.md` 등 스킬 파일)는 t6 구현이 반영되지 않은 옛 상태였지만, `role-terminal` 자신의 옵션 파싱·`firstPrompt` 조합·launch 판정 로직은 t6 구현이 반영된 런타임으로 실행됐다.

### 실측으로 확정한 것

Claude Code(v2.1.283, 2026-09-26)에서 `role-terminal --brief`로 연 PM 세션의 화면에는 첫 프롬프트가 붙여넣기 표시 없이 일반 입력줄(`❯`)로 나타났고, 세션 기록(jsonl)의 첫 사용자 메시지도 `content`가 `<pasted_content>` 래핑 없는 순수 문자열이었으며 `promptSource: "typed"`, `origin.kind: "human"`, `turnOrigin: "human"`으로 남았다. PM은 사용자 확인을 구하지 않고 AGENTS.md를 읽은 뒤 브리프가 지시한 대로 "브리프 확인 완료"라고만 답하고 멈췄다. 명령 결과는 `ready: true`, `submission: "enter-sent"`였다. 이 `enter-sent`는 PM 세션 안에서 브리프 내용에 대해 role-terminal이 별도로 보낸 Enter가 아니다: `role-terminal.mjs`의 `launchOnce`(717~723행 `commandPending` 분기)는 셸이 방금 그 launch 명령(브리프 문장을 인자로 실은 실행 명령 자체)을 아직 실행하지 않고 화면 끝에 그대로 echo한 상태를 스스로 판정했을 때에만 빈 텍스트와 Enter를 보내며, 이번 확인에서도 그 판정 뒤 한 번만 나갔다. 즉 이 Enter가 향한 대상은 PM 세션을 여는 launch 명령이며, 브리프의 내용 자체가 아니다.

이 확정 범위는 이 CLI 판(Claude Code v2.1.283)으로 한정한다. 붙여넣기 판정·Enter 대상·세션의 첫 턴 여부라는 이 관찰 사실 자체는 `role-launch.mjs`가 실행 명령의 위치 인자로 첫 프롬프트를 넘기는 방식과 Claude Code 자신의 판정 동작에서 나오므로, 세션이 어느 워크트리에서 열렸는지와 무관하다(위 「실행 조건」). Codex와 Agy는 여전히 `--help`로 인자·플래그의 존재만 확인했을 뿐, 세션에서의 표시는 관찰하지 않았다(위 「검토한 세 가지 선택지」).

### 실측하지 않은 것

- **Goal·Run·kickoff-bind**: 확인용 브리프가 어떤 명령도 실행하지 말고 Goal·Run·kickoff-bind도 만들지 말라고 지시했으므로, PM이 브리프를 사용자 확인 없이 따른 뒤 실제로 Goal을 만들고 kickoff-bind까지 진행하는 경로는 실측하지 않았다.
- **새 pm 스킬 규칙이 적용된 세션의 동작**: 위 「실행 조건」에서 밝혔듯 이 세션이 연 워크트리의 `pm/SKILL.md`에는 이 rework가 추가한 턴 순서 신뢰 조건 절 자체가 없었다. 그러므로 PM이 사용자 확인 없이 브리프를 따른 것이 이 새 규칙 덕분인지, 그 절 자체가 없는 상태에서 도착한 첫 지시를 그대로 따른 것뿐인지는 이 확인만으로 가려낼 수 없다. 새 규칙이 실제로 적용된 환경에서의 PM 동작은 관찰하지 못했다.
- **그 세션이 실제로 읽은 플러그인·스킬 경로**: 이 확인이 남긴 기록(명령 결과, 화면 원문, jsonl 메타데이터)의 확인 범위 안에서는, PM이 AGENTS.md를 읽었다는 것 외에 어느 경로의 스킬 파일을 읽었는지 확정할 수 없다.
- **등록부에 없는 터미널이나 다른 브리프 경로를 담은 텍스트를 따르지 않는다는 부정 조건(ac-2)**: 이 확인은 매칭되는 경로만 실행했고 이 부정 조건은 실측하지 않았다. 이 조건의 판정(`verifyHandoffClaim`의 `matched`·`mismatch`·`not-registered`·`no-director-recorded`)은 대신 임시 등록부를 쓰는 가짜 orca 회귀 테스트가 덮는다: `tests/kickoff-registry.test.mjs`의 "a pasted handoff claim is trusted only when it matches the registered director (#86)"가 이 네 가지 판정(등록 전 `not-registered`, director 미기록 `no-director-recorded`, 터미널·브리프 경로가 다른 두 `mismatch`, 일치하는 `matched`)을 모두 재현하고, `tests/runtime.test.mjs`의 "kickoff-handoff-verify CLI command reports whether a pasted claim matches the registry (#86)"가 `matched`·`mismatch`를 CLI 레벨에서 다시 확인한다. 두 테스트 모두 임시 디렉터리에 쓴 파일 기반 등록부만 다루며 실제 CLI를 띄우지 않는다.
- 확인이 끝난 뒤 그 PM 터미널을 `orca terminal close`로 닫고 임시 폴더를 지웠다. 이 CLI 판(Claude Code v2.1.283)의 이 worktree 신뢰 상태에서는 폴더 신뢰 질문이 뜨지 않아(`trust: "not-asked"`), 신뢰 질문이 있는 경로는 이번에 관찰하지 못했다.
