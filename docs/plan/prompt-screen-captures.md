...
- Codex와 Claude 신뢰 질문에서 Enter의 효과: 캡처 중에 Enter를 보낸 적이 없어 Enter를 누르면 질문이 사라지고 에이전트 화면이 나타나는지 확인하지 못했습니다. 분류기는 화면의 안내 문구("Press enter to continue", "Enter to confirm")만을 근거로 이 키를 `footer-text`로 표시하며, 보낸 뒤 화면을 다시 읽어 질문이 사라졌는지 확인해야 합니다.
- Agy 신뢰 질문 화면의 오른쪽 정렬 상태 줄이 시작하는 열: 캡처는 한 시점의 한 창 너비에서만 얻었습니다. 분류기는 선택지 아래에서 공백 10칸 이상으로 시작하는 줄을 상태 줄로 보는데, 이 기준은 관측값이 아니라 정한 값이므로 창 너비가 좁아져 상태 줄이 그보다 왼쪽에서 시작하면 질문을 답하지 않는 쪽으로 판정합니다.
