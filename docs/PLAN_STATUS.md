...
| [oh my teams 이름 변경과 Orca 연동](plan/oh-my-teams-rename-and-orca-integration.md) | R0–R4 | package·marketplace·manifest의 기본 버전 1.6.0 일치, `plugins/orca` forwarding 진입점, Orca 1.4.200 discovery, 설치·CLI 회귀 테스트 | 이전 0.6.1은 rollback과 역사 보존 용도로만 유지한다. |
...
2026-09-15 현재 작업 트리에서 다음 검증을 다시 실행했다.

| 명령 | 결과 |
|---|---|
| `npm test` | 81개 테스트가 모두 통과했다. |
| `npm run quality` | 활성 `.mjs` 40개와 공개 export 117개를 검사했으며 지적 사항이 없었다. |
