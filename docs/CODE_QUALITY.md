...
아래 수치는 초기 정규식 검사 결과다. 모든 매개변수 설명의 정확성이나
JavaScript 문법 전체를 검증한 수치로 해석하면 안 된다. 후속 검사에서
멀티라인·구조분해 인자와 export arrow 함수, template이 섞인 긴 실행문에
대한 누락을 발견해 회귀 테스트와 검사를 보강했다. 새 검사 결과는
`npm run quality`의 실제 출력으로 확인한다.

| 항목 | 결과 |
|---|---:|
| 활성 `.mjs` 파일 | 34/34 모듈 문서화 |
| 공개 export | 87/87 JSDoc |
| 초기 검사에서 인식한 매개변수 태그 | 70개 (정확한 전체 인자 커버리지는 미측정) |
| 공개 함수 | 78/78 `@returns` |
| 오류 계약을 가진 공개 API | 61개 `@throws` 명시 |
| 장문 실행문·문서 누락 | 0건 |
